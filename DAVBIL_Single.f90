...
DOUBLE PRECISION :: TMPE,RAND, DELTA, DELTAMIN, KEY, KE, KEYMAX, TAU, ACOEFF, BCOEFF, &
		    EPS, NU, UCAP, ARGUMENT, DELTAKEY, LT, ENERGY, KETA, TRACE, URMS, KIN, VAR, MEAN
REAL :: rand_number
...
KE = 1.453D0 * 9.0D0 * PI / (55.0D0 * LT)
DELTAKEY = (KEYMAX - KE/2.0D0) / (ENNE - 1.0D0)
...
!------------BEGINNING OF EDDIES ITERATIONS
	...
	VELFLU(1) = VELFLU(1) + UCAP * COS(ARGUMENT) * SIGMA_P(1)
	VELFLU(2) = VELFLU(2) + UCAP * COS(ARGUMENT) * SIGMA_P(2)
	VELFLU(3) = VELFLU(3) + UCAP * COS(ARGUMENT) * SIGMA_P(3)
