...
DOUBLE PRECISION, DIMENSION(:), ALLOCATABLE ::          K, PHI, PSI, ALPHA, THETA, KNORM, KNORM_P, U_N,FVEC
...
DOUBLE PRECISION :: TMPE,RAND, DELTA, DELTAMIN, KEY, KE, KEYMAX, TAU, ACOEFF, BCOEFF, &
		    EPS, NU, ARGUMENT, DELTAKEY, LT, ENERGY, KETA, TRACE, URMS, KIN, PDAVBIL, &
		    SCALINGPARAM, AMP, K1, F
REAL :: rand_number
...
LOGICAL	     ::     ISPRINT, ISDEBUG, ISFIXMAT
...
!REYNOLDS = (/ 7.66D0, -0.66D0, 3.19D0, 0.0D0, 0.0D0, 1.49D0 /)  ![M/S]
!REYNOLDS = (/ 0.76D0, -0.66D0, 1.09D0, 0.0D0, 0.0D0, 1.49D0 /)  ![M/S]
!REYNOLDS = (/ 1.66D0, -0.66D0, 0.319D0, 0.0D0, 0.0D0, 5.49D0 /)  ![M/S]
...
SCALINGPARAM = 1.0D0 !SQRT(10.0D0)
...
IF (ISFIXMAT) THEN
 ...
 TMPVEC = EIGVAL
 EIGVAL = (/TMPVEC(3), TMPVEC(1), TMPVEC(2)/)
...
DO II=1,N
  ...
  KAP(1,II) = SIN(THETA(II)) * COS(PHI(II))
  KAP(2,II) = SIN(THETA(II)) * SIN(PHI(II))
  KAP(3,II) = COS(THETA(II))

  KNORM(II) = KEY
  KAP(:,II) = KAP(:,II) * KNORM(II)

  K_P(:,II) = MATMUL(ROTT,KAP(:,II)) * ISQRTEIGVAL
  KNORM_P(II) = SQRT(K_P(1,II)**2.0D0 + K_P(2,II)**2.0D0 + K_P(3,II)**2.0D0)
...
!  ENERGY = AMP * URMS**2.0D0 / KE * ( KNORM_P(II) / KE)**4.0D0 / &
!	   (1.0D0 + (KNORM_P(II)/KE)**2.0D0)**(17.0D0/6.0D0) * &
!	   EXP(-2.0D0 * ( KNORM_P(II)/KETA )**2.0D0)

  TMPE = KNORM(II)

  ENERGY = AMP * URMS**2.0D0 / KE * ( TMPE / KE)**4.0D0
  ENERGY = ENERGY / (1.0D0 + (TMPE/KE)**2.0D0)**(17.0D0/6.0D0)
  ENERGY = ENERGY * EXP(-2.0D0 * ( TMPE/KETA )**2.0D0)

  U_N(II) = SQRT(ENERGY * DELTAKEY)	
...
!------BEGINNING OF SPATIAL ITERATION
  ...
  !     IY = MOD(KIDX-1,DIVY)+1
   !    IZ = (KIDX-1)/DIVY + 1
	...
	IF (KNORM_P(II) < KEYMAX) THEN
				
	  ARGUMENT = DOT_PRODUCT(K_P(:,II),X_POINT_P) + PSI(II)

	  F = SCALINGPARAM * 2.0D0 * U_N(II) * COS(ARGUMENT)
	  FVEC(II) = F

	  VELFLU = VELFLU +  F * SIGMA_P(:,II)
	...
	OPEN (UNIT=66, FILE='debug.dat',FORM='unformatted', ACCESS="stream") 
	WRITE(66) KNORM,KNORM_P,U_N,FVEC

	PRINT *, "EXITING PROGRAM, WRITING DEBUG INFORMATION TO debug.dat"
	CALL EXIT(IERR)

	END IF	

	VELFLU = MATMUL(ROT,VELFLU)
	!VELFLU = MATMUL(VELFLU,ROT)
