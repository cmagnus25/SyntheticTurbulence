...
ALLOCATE(X_EDDY(3,N_part))
ALLOCATE(EPSILO(3,N_part))
ALLOCATE(MOLT(3,N_part))
ALLOCATE(K(N_part))

!IF (MYRANK == 0) THEN
ALLOCATE(DATAVEC(ITMAX,15))
ALLOCATE(V_GLOB(DIVY,DIVZ,3))
!END IF
...
!PRINTINGS OF GLOBAL VELOCITY AND OF CONVECTION VELOCITY
  IF ((IT==1 .OR. MOD(IT,P)==0 .OR. IT==ITMAX) .AND. MYRANK == 0 ) THEN
...
!-----------END OF EDDIES ITERATIONS
      ...
      V(IY,IZ,:) = (V(IY,IZ,:) / SQRT(ENNE)) ! + U(:)/(FLOAT)N_part
...
!THIS IS THE POST-PROCESSING PART, ACTIVATED ONLY IF [POST_PROCESSING] == 1
  IF (POST_PROCESSING == 1 .AND. MYRANK == 0) THEN
    !PRINT *, "Start postprocessing"
    ! Add the mean velocity to the fluctuating field only on master process
    VEL_AVERAGE(:) = (V_GLOB(IDXY,IDXZ,:) + VEL_AVERAGE(:) * (IT-1)) / IT

    VEL2_AVERAGE(:) = ((/V_GLOB(IDXY,IDXZ,1)**2,V_GLOB(IDXY,IDXZ,2)**2,V_GLOB(IDXY,IDXZ,3)**2, &
V_GLOB(IDXY,IDXZ,1)*V_GLOB(IDXY,IDXZ,2), V_GLOB(IDXY,IDXZ,1)*V_GLOB(IDXY,IDXZ,3), & 
V_GLOB(IDXY,IDXZ,2)*V_GLOB(IDXY,IDXZ,3) /) + VEL2_AVERAGE(:) * (IT-1)) / IT
